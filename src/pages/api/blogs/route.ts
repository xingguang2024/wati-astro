...
export const GET: APIRoute = async ({ request, locals, url }) => {
  ...
  const userBlogs = await db.select().from(blogs).where(
    and(
      eq(blogs.userId, payload.userId),
      status !== 'all' ? eq(blogs.status, status) : undefined
    ) as any
  ).orderBy(desc(blogs.updatedAt))

  return new Response(
    JSON.stringify({ blogs: userBlogs }),
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const newBlog = await db.select().from(blogs).where(eq(blogs.id, blogId)).limit(1)

  return new Response(
    JSON.stringify({ blog: newBlog[0] }),
